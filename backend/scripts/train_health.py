"""
猫健康状态分类训练脚本 (Healthy / Sick 二分类)
基于 Ultralytics YOLOv11 目标检测
预训练模型: Model 1 (models/cat_breeds/best.pt) 迁移学习

数据集: E:/program/health (YOLO 格式, 1065 train / 207 val / 268 test)
训练记录和图表将保存在 runs/train_health/ 目录下
"""
...
DATASET_PATH = "E:/program/health/data.yaml"
DEFAULT_EPOCHS = 100       # 先试 100 轮，根据效果调整
...
DEFAULT_PROJECT = str(BACKEND_DIR / "runs" / "train_health")
...
def plot_training_curves(history: dict, save_dir: str, class_names: list = None):
    ...
    plt.suptitle("Health Classification — Training Overview", fontsize=14, fontweight="bold")
...
def train_yolo(
    ...
    # ── 初始化模型 ──
    print(f"\n{'='*60}")
    print(f"  猫健康状态分类训练启动 (2 类: Healthy / Sick)")
    print(f"{'='*60}")
    ...
    results = model.train(
        ...
        deterministic=False,
        amp=True,
        cls_pw=0.7,       # Sick 类权重略高（范围0~1，0=自动），提升召回率
        mixup=1.0,        # 开启 mixup 增强，缓解类别不平衡
    )
    ...
    if best_pt.exists():
        import shutil
        final_model_path = save_dir.parent / "best_health_model.pt"
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="YOLOv11 猫健康状态分类训练 (Healthy / Sick)",
