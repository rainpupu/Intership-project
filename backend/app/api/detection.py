...
from app.entity.schemas import ApiResponse, AnalyzeRequest, AnalyzeResponse
...
encounter_router = APIRouter(prefix="/api/encounters", tags=["猫咪识别"])


@encounter_router.post("/{encounter_id}/analyze", response_model=ApiResponse)
async def analyze_encounter(
    encounter_id: int,
    db: Session = Depends(get_db),
    test_mode: bool = False,
    # TODO: 上线前恢复为 Depends(get_current_user)
    ...
    【测试模式】test_mode=true 时，不查数据库，直接用本地 test_cat.jpg 跑模型。
    encounter_id 可随便填（如 1）。
    """
    # ── 测试模式：直接跑本地图片，返回完整检测结果（含坐标+裁剪图） ──
    if test_mode:
        result = vision_service.test_predict("test_cat.jpg")
        return ApiResponse(data=result)

    result = await vision_service.analyze_encounter(
        db=db,
