...
def _match_knowledge(query: str, kb: dict[str, str]) -> str | None:
    ...
    # 需要至少 2 个字符重叠才视为命中
    if best_score >= 2 and best_key and best_key in kb:
        return kb[best_key]
...
def search_cats(keyword: str = "", coat_color: str = "", adoption_status: str = "", limit: int = 10) -> str:
    ...
    results = _MOCK_CATS
    if keyword:
        results = [c for c in results if keyword in c["name"] or keyword in c["code"]]
    if coat_color:
        results = [c for c in results if coat_color in c["coat_color"]]
    if adoption_status:
        results = [c for c in results if c["adoption_status"] == adoption_status]
...
def get_cat_observations(cat_id: int, limit: int = 10) -> str:
    """获取指定猫咪的状态观察记录时间线。"""
    records = [o for o in _MOCK_OBSERVATIONS if o["cat_id"] == cat_id][:limit]
    if not records:
        return "暂无该猫咪的状态记录。"
    lines = []
    for r in records:
        lines.append(
            f"- [{r['observed_at']}] 情绪：{r['mood_status']} | 健康：{r['visible_health_status']} | {r['notes']}"
        )
...
def get_recent_encounters(cat_id: int = 0, days: int = 30) -> str:
    ...
    records = _MOCK_ENCOUNTERS
    if cat_id > 0:
        records = [e for e in records if e["cat_id"] == cat_id]
    if not records:
        return "近期没有出现事件记录。"
    lines = []
    for r in records:
        lines.append(f"- [{r['occurred_at']}] 地点：{r['location']}（{r['image_count']} 张图片，{r['status']}）")
...
def recommend_adoption_cats(personality: str = "", experience: str = "", limit: int = 3) -> str:
    """根据用户偏好推荐适合领养的猫咪。"""
    candidates = [c for c in _MOCK_CATS if c["adoption_status"] != "已领养"]
    if personality:
        candidates = [c for c in candidates if personality in c["personality_tags"]]
    if not candidates:
        candidates = [c for c in _MOCK_CATS if c["adoption_status"] != "已领养"]
    ...
    for cat in results:
        lines.append(
            ...
            f"：{cat['personality_tags']}，{cat['description']}"
        )
    if experience == "新手":
        lines.append("\n温馨提示：作为新手领养人，建议选择性格亲人、成年的猫咪，它们通常更稳定好养。")
    return f"为您推荐 {len(results)} 只猫咪：\n" + "\n".join(lines)
...
def get_attention_cats(limit: int = 10) -> str:
    ...
    cats = []
    for cat in _MOCK_CATS:
        obs = [o for o in _MOCK_OBSERVATIONS if o["cat_id"] == cat["cat_id"]]
        health_issue = any(o["visible_health_status"] in ("需观察", "异常") for o in obs)
        if health_issue:
            cats.append((cat, "健康需关注"))
    if not cats:
        return "目前所有猫咪状态良好，无需特别关注。"
    lines = []
    for cat, reason in cats[:limit]:
        lines.append(f"- {cat['name']}：{reason}，最近出现：{cat['last_seen_at']}")
