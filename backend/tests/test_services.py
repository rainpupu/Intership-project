...
class TestUserRegister:
    ...
    def test_register_duplicate_username(self, db):
        ...
        with pytest.raises(HTTPException) as exc:
            user_service.register(db, "testuser", "b@example.com", "password123")
    ...
    def test_register_duplicate_email(self, db):
        ...
        with pytest.raises(HTTPException) as exc:
            user_service.register(db, "user2", "test@example.com", "password123")
        assert exc.value.status_code == 400
        assert "邮箱已被注册" in exc.value.detail

    def test_register_minimal_password(self, db):
        """6 位密码注册成功"""
        user = user_service.register(db, "minimal", "min@example.com", "123456")
        assert user.id is not None
...
class TestUserRoles:
    ...
    def test_get_user_by_id(self, db):
        """根据 ID 获取用户"""
        created = user_service.register(db, "byid", "byid@example.com", "password123")
...
class TestTokenGeneration:
    ...
    def test_create_access_token_for_user(self, db):
        """为用户生成 JWT Token"""
        user = user_service.register(db, "tokenuser", "token@example.com", "password123")
